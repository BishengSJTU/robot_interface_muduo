...
PlatformPLC::PlatformPLC(std::string address, int port, bool isInline)
{
    if(!isInline)
        return;
...
void PlatformPLC::InitializePLC(std::string address, int port)
{
    address_ = address;
    port_ = port;
    std::stringstream io;
...
PlatformPLC::~PlatformPLC(){
    LOG_INFO << "MyPLC析构";
    close(clientSocket_);
}

// PLC动作
bool PlatformPLC::ControlPLC(int command) {
...
// 判断档案是否取到末端中
bool PlatformPLC::ArchiveInHand() {
...
void PlatformPLC::Move(int x_delta, int y_delta) {
    int x_current, y_current, reset_state, archive_state, sensor_state;
    std::cout << "x_delta:" << x_delta << ", y_delta:" << y_delta << std::endl;
    GetState(x_current, y_current, reset_state, archive_state, sensor_state);
...
//    std::cout <<  std::hex << (unsigned int)(unsigned char)y_H4 << "," << (unsigned int)(unsigned char)y_H3 << "," << (unsigned int)(unsigned char)y_H2 << ","<< (unsigned int)(unsigned char)y_H1 << ","<< std::endl;
    ...
    if(send(clientSocket_, set_x, sizeof(set_x), 0) < 0) {
        LOG_ERROR << "PLC客户端发送数据错误，异常退出！";
        exit(PLCEXITNUM);
    }
//    std::cout << "send x_abs:";
...
//    std::cout << std::endl;
    ...
    set_y[13] = y_H2;
    set_y[14] = y_H1;
    if(send(clientSocket_, set_y, sizeof(set_y), 0) < 0) {
        LOG_ERROR << "PLC客户端发送数据错误，异常退出！";
        exit(PLCEXITNUM);
    }
//    std::cout << "send y_abs:";
...
//    std::cout << std::endl;
    ...
    if(send(clientSocket_, set_direction, sizeof(set_direction), 0) < 0) {
        LOG_ERROR << "PLC客户端发送数据错误，异常退出！";
        exit(PLCEXITNUM);
    }
...
//    std::cout << std::endl;
    ...
    while(1)
    {
        GetState(x, y, reset, archive, sensor);
...
void PlatformPLC::PickArchive(int arm_position)
{
    ManipulateArchive(arm_position, 1);
}
void PlatformPLC::PlaceArchive(int arm_position)
{
    ManipulateArchive(arm_position, 2);
}

void PlatformPLC::ManipulateArchive(int arm_position, int mode)
{
    ...
    if(send(clientSocket_, set_position, sizeof(set_position), 0) < 0) {
        LOG_ERROR << "PLC客户端发送数据错误，异常退出！";
        exit(PLCEXITNUM);
    }
    ...
    if(send(clientSocket_, set_mode, sizeof(set_mode), 0) < 0) {
        LOG_ERROR << "PLC客户端发送数据错误，异常退出！";
        exit(PLCEXITNUM);
    }
...
//    std::cout << std::endl;
    ...
    while(1) {
        GetState(x, y, reset_state, archive, sensor);
        if(reset_state == 0x02)
            break;
    }
}

void PlatformPLC::GetState(int &x_current, int &y_current, int &reset_state, int &archive_state, int &sensor)
{
    unsigned char buff_get_state[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x01, 0x03, 0x00, 0x0A, 0x00, 0x08};
    unsigned char buff_state[25];
//    std::cout << "send:";
//    for(int i = 0; i < sizeof(buff_get_state); i++)
//        std::cout  << std::hex << (unsigned int)(unsigned char)buff_get_state[i] << ",";
//    std::cout << std::endl;
    if(send(clientSocket_, buff_get_state, sizeof(buff_get_state), 0) < 0) {
        LOG_ERROR << "PLC客户端发送数据错误，异常退出！";
        exit(PLCEXITNUM);
    }
